    ...
    <div>
      {startingEquipment && (
        <div className="border">
          <div className="jumpTarget" id="starting-equipment">
            <strong>Starting Equipment: </strong>
            {startingEquipment.starting_equipment.map((e) => {
              return (
                <div>
                  <br />
          ...
          <div>
            {startingEquipment.starting_equipment_options.map(
              (typeOfEquipment) => {
                const fromIsArray =
                  !!typeOfEquipment.from &&
                  typeOfEquipment.from.constructor === Array;
                const fromIsObject =
                  !!typeOfEquipment.from &&
                  typeOfEquipment.from.constructor === Object;
                      ...
                      <div>
                        {typeOfEquipment.from.map((equipment) => {
                          if (equipment.equipment !== undefined) {
                            return <div>{equipment.equipment.name}</div>;
                          } else if (equipment.equipment_option !== undefined) {
                            return (
                              <div>
                                {
                                  equipment.equipment_option.from
                                    .equipment_category.name
                                }
                              </div>
                            );
                          } else if (
                            typeOfEquipment.from[0].length > 0 &&
                            typeOfEquipment.from[1].length === undefined
                          ) {
                            console.log(typeOfEquipment.from[0].length);
                            if (equipment.equipment !== undefined) {
                              return <div>{equipment.equipment.name}</div>;
                            } else if (
                              equipment.equipment_option !== undefined
                            ) {
                              return (
                                <div>
                                  {
                            ...
                            return <div>There seems to be a problem</div>;
                          }
                        })}
                      </div>
                    </div>
